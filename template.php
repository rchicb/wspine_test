...
// Add Zen Tabs styles
if (theme_get_setting('wspine_test_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'wspine_test') .'/css/tabs.css');
}
...
function wspine_test_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];

  // Merge first/last class (from wspine_test_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
 ...
 */
function wspine_test_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('wspine_test_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('wspine_test_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('wspine_test_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('wspine_test_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('wspine_test_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */	
function wspine_test_id_safe($string) {
 ...
 * 
 */
function wspine_test_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = wspine_test_id_safe($element['#title']);
 ...
 */
function wspine_test_preprocess_menu_local_task(&$variables) {
 ...
 */
function wspine_test_menu_local_tasks(&$variables) {  
  $output = '';
